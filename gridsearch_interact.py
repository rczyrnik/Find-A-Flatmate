...
def oversample(X, y, tp):
    ...
    RETURNS         X_undersampled : ndarray - 2D
...
def gridsearch_gbc(X, y, resample_wt=.5,
    ...
    NPV = get_NPV(confusion_matrix(y_test, y_pred))

    f1 = (recall*precision)/(recall + precision)
    print("resample_wt: {}, max_depth: {}, f1: {:.3},   rcll: {:.3},   prcsn: {:.3}, NPV: {}".format(resample_wt,max_depth,f1,recall,precision, NPV))

    return recall, precision, NPV
...
if __name__ == "__main__":
    ...
    date = '013018'
    try:
        X = pd.read_json('/Users/gandalf/Documents/data/data_X_'+date+'.json')
        y = pd.read_json('/Users/gandalf/Documents/data/data_y_'+date+'.json')['interact']
    except:
        X = pd.read_json('data_X_'+date+'.json')
        y = pd.read_json('data_y_'+date+'.json')['interact']
    ...
    for max_depth in range(1, 10):
        for resample_wt in np.arange(.1,.9,.05):
            recall, precision, NPV = gridsearch_gbc(X, y, resample_wt=resample_wt,
                                                    loss='exponential',
                                                    learning_rate=.025,
                                                    max_depth=max_depth,
                                                    min_samples_split=3,
